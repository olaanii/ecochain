...
test("middleware should identify protected routes correctly", () => {
  ...
  testCases.forEach(({ path, expected }) => {
    ...
    assert.strictEqual(
      isProtected,
      expected,
      `Path ${path} should ${expected ? "be" : "not be"} protected`
    );
  });
});

test("middleware logic: protected routes take precedence", () => {
  const protectedRoutes = [
    "/dashboard",
    "/verification",
    "/merchants",
  ...
  testCases.forEach(({ path, shouldRedirectIfNotAuth }) => {
    ...
    assert.strictEqual(
      isProtected,
      shouldRedirectIfNotAuth,
      `Path ${path} should ${shouldRedirectIfNotAuth ? "redirect" : "not redirect"} unauthenticated users`
    );
  });
});
