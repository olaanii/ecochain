    ...
    fc.assert(
      fc.property(
        ...
        (principal) => {
          ...
          // Verify penalty is approximately 10% of principal (accounting for integer division)
          const penaltyPercentage = (penalty * BigInt(100)) / principal;
          // Due to integer division, we expect 9 or 10
          expect(penaltyPercentage).toBeGreaterThanOrEqual(BigInt(9));
          expect(penaltyPercentage).toBeLessThanOrEqual(BigInt(10));
        }
      )
    );
  });
